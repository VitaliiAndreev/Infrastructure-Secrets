...
```powershell
RequiredModules = @(
    @{ ModuleName = 'PowerShell.Common'; ModuleVersion = '1.1.0'; ... }
)
```

However:

- `PowerShell.Common` has dropped PS 5.1 support and bumped to `2.0.0` (see
  [that repo's implementation 03](../../../../../../../PowerShell-Common/docs/dev/implementation/03%20-%20drop%20ps51/problem.md)).
...
- The CI unit-test workflow (`ci.yml`) delegates to
  `PowerShell-Common/.github/workflows/ci-powershell.yml`, which no longer contains
...
Unlike `PowerShell.Common`, this module contains **no PS 5.1 compatibility
...
| `Infrastructure.Secrets.psd1` | `RequiredModules` pins `PowerShell.Common 1.1.0` | Pinned to `2.0.0` (first PS 7-only release) |
...
from `PowerShell-Common` automatically.

---

## Why Now

- `PowerShell.Common 2.0.0` no longer supports PS 5.1. Continuing to declare
  `PowerShell.Common >= 1.1.0` as the required version would allow PSGallery to
...
- No test changes - tests are already PS 7-compatible (integration tests run in Docker
  on PS 7 only).
- No `ci.yml` changes - the PS 5.1 job removal propagates from PowerShell-Common.
